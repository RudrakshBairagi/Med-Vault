      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5 ml-1">ABHA or Aadhaar Number</label>
              <input 
                type="text" 
                required
                placeholder="12-digit Aadhaar or 14-digit ABHA"
                value={abhaId}
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5 ml-1 mr-1">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">OTP</label>
                {isLogin && <a href="#" className="text-xs font-bold text-primary hover:underline">Resend OTP</a>}
              </div>
              <input 
                type="text" 
                required
                ...
                maxLength={6}
                className="w-full bg-surface-container-lowest border border-outline-variant/50 rounded-xl px-4 py-3.5 text-sm text-[#01261f] font-medium focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:text-[#01261f]/30 tracking-widest text-center" 
              />
            </div>
          ...
          <div className="space-y-3">
            <button onClick={handleOAuth} disabled={isLoading} className="w-full bg-[#005696] text-white rounded-xl py-3.5 flex items-center justify-center gap-3 hover:bg-[#005696]/90 transition-colors active:scale-[0.98] shadow-lg shadow-[#005696]/20">
              <span className="material-symbols-outlined font-bold text-xl">security</span>
              <span className="text-sm font-bold tracking-wide">Continue with DigiLocker</span>
            </button>
          </div>
